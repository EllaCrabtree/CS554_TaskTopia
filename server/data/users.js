...
async function createUser(firstName, lastName, username, password, email, uid) {
  ...
  if (!uid) {
    throw "Error: UID not supplied!";
  }

  if (typeof uid !== "string") {
    throw "Error: UID must be a string!";
  }

  if (uid.trim().length === 0) {
    throw "Error: UID cannot be empty or only spaces!";
  }
...
async function getUserByUID(uid) {
  ...
  if (typeof uid !== "string") {
    throw "Error: UID must be of type string!";
  }

  if (uid.trim().length === 0) {
    throw "Error: UID cannot be all spaces or empty!";
  }
...
async function addBuildingToUser(username, building) {
  ...
  if (typeof buildingID !== "string")
    throw "Error: Building ID must be of type string!";
  if (buildingID.trim().length === 0)
    throw "Error: Building ID cannot be all spaces or empty!";

  const newBuildingID = buildingID.trim();

  if (!ObjectId.isValid(newBuildingID))
    throw "Error: Building ID is not a valid ObjectID!";
...
async function updateLevel(username, newLevel) {
  ...
  if (!Number.isInteger(newLevel))
    throw "Error: New Level must be of type int!";
...
async function addFriend(username, friendUsername) {
  ...
  if (typeof friendUsername !== "string")
    throw "Error: friendUsername must be a string!";

  if (friendUsername.trim().length === 0)
    throw "Error: friendUsername cannot be empty or only spaces!";

  // Check username requirements - decide later
  const new_friendUsername = friendUsername.trim().toLowerCase();

  //check if username is within database
  const foundFriendUser = await userCollection.findOne({
    username: new_friendUsername,
  });
  ...
  const updatedInfo2 = await userCollection.updateOne(
    ...
    { $set: { friends: new_friends_friendUser } }
  );
...
async function deleteUser(userId) {
  ...
  if (!ObjectId.isValid(userId))
    throw "Error: UserID given is an invalid object ID";
